...
| 18 | 控制端 PKI 前置到 init(2026-09-01 定,基线后增量) | **CA 生成从 `server` 启动中独立出来**:`init` 末尾交互式设置控制端 CA + 服务器证书,SAN 的 **DNS 与 IP 分开询问**(`--san-dns`/`--san-ip` 供非交互);已有 CA 保留(重生 CA = 全量重置),服务器证书 SAN 已覆盖则复用、否则重签(`certs.IssueServerCert`)。解决通配 `--api` 绑定导致 SAN 缺公网 IP、需手动删 `server.pem` 重签的问题 |
