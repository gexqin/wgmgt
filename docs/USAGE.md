...
| | `--force-enroll` | `false` | 用 token 替换已有证书材料(重新注册命令自动带上) |
...
fwmark 标记隧道自身流量、默认路由进每接口独立的路由表、
...
- 超时未验证:自动**回退**(停掉全部受管接口、清理策略路由),
  并进入**隔离**:该配置修订号不再自动重试(避免"回退→重锁"死循环;
  控制端数据库恢复导致修订号回退时也允许恢复)
...
- agent 证书 CN 即客户端名;注册端点允许无客户端证书,但 poll handler
  会拒绝无证书或指纹已被替换的请求
- 控制端持有所有客户端的接口私钥(集中式配置管理的固有代价)
- agent 仅保留证书、生成的 conf 和自锁隔离修订号,重启即重新收敛
